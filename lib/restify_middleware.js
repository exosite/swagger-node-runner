  ...
  this.register = function register(app) {
    ...
    ALL_METHODS.forEach(function(method) {
      app[method]('.*', function(req, res, next) {
        // requires to add app.use(restify.queryParser()); upfront to avoid sway module to raise a missing req.query
        // req.query = req.query(); // oddly, req.query is a function in Restify, kill it
